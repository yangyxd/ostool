<template>
  <div class="regexp container">
    ...
    <el-row class="mt8">
      <div :style="{ width: 'calc(100% - 450px)', minWidth: isMobile() ? '100%' : '520px' }"  :span="16">
        <div>
        ...
        <el-row class="mt16">
          <div style="width: calc(100% - 142px); float: left">
            <el-input placeholder="输入替换文本或值索引号" clearable v-model="regReplace"></el-input>
          </div>
          <div style="text-align: right; width: 138px; float: right">
      ...
      <div style="width: 450px">
        <div class="rightpl">
          ...
          <div v-for="(item, index) in templates" :key="index" :title="item[1]" class="titem" @click="regKey = item[1]">
            <span>{{ item[0] }}</span>
          </div>
        </div>
      </div>
    ...
    execMatch() {
      try {
        ...
        if (this.matchAll) {
          ...
          for (let i = 0; i < result.length; i++) strResult += result[i] + '\r\n'
