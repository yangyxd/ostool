<template>
  <div class="container xpath full">
    <el-row>
      <el-col>
        <div class="e src">
          <codemirror v-model="textarea" placeholder="请输入要格式化 或者 需要压缩的Js/Html字符串" :style="{ padding: '0px !important',
            height: mobile ? 'calc(100vh - 136px)' : 'calc(100vh - 95px)' }" :indent-with-tab="true" :tab-size="4" :extensions="cmExt" />
        </div>
        <el-row style="text-align: center; padding: 5px 12px 4px 12px; width: 100%; display: inline-block; line-height: 40px; background-color: #303030;">
          ...
          <el-button type="success" @click="compressEx()">加密压缩</el-button>
          <el-select v-model="optionValue" placeholder="请选择" style="margin-left: 8px; margin-right: 8px; width: 150px">
            <el-option v-for="item in options" :key="item[1]" :label="item[0]" :value="item[1]"> </el-option>
          </el-select>
          <el-button type="danger" @click="textarea = ''">
            ...
            <span style="padding-left: 2px">清空</span></el-button>
          <el-button type="success" @click="onCopyOK(textarea)">复制代码</el-button>
        </el-row>
      </el-col>
    </el-row>
  </div>
...
`
  ...
  methods: {
    ...
    format() {
      try {
        let tabchar = ' '
        if (this.optionValue == 0) tabchar = '\t'
        let js_source = this.textarea.replace(/^\s+/, '')
        if (js_source && js_source.charAt(0) === '<') {
          // html
          this.textarea = this.formatHtml(js_source, this.optionValue, tabchar, 80)
        } else {
          // js
          this.textarea = this.formatJs(js_source, this.optionValue, tabchar)
        }
      } catch (e) {
    ...
    compressEx() {
      ...
      this.textarea = packer.pack(this.textarea, 1, 0)
    },
    compress() {
      // eslint-disable-next-line no-undef
      var packer = new Packer()
      this.textarea = packer.pack(this.textarea, 0, 0)
    },
    formatHtml(source, tabsize, tabchar, w) {
      return this.style_html(source, tabsize, tabchar, w)
    },
    formatJs(source, tabsize, tabchar) {
      return this.js_beautify(source, tabsize, tabchar)
    },
      ...
      function Parser() {
        ...
        this.get_token = function () {
          ...
          if (this.last_token === 'TK_TAG_SCRIPT') {
            ...
            token = _this.js_beautify(temp_token, this.indent_size, this.indent_character, this.indent_level) //call the JS Beautifier
