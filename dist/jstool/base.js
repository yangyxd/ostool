...
new function(_y) {
    ...
    ;function instanceOf(a, b) {
        ...
        if (a.constructor == b)
            return true;
        if (b.ancestorOf)
            return b.ancestorOf(a.constructor);
...
new function() {
    ...
    global.Packer = Base.extend({
        ...
        pack: function(a, c, d, b) {
            ...
            if (d)
                a = this.shrinker.shrink(a);
            if (b)
                a = this.privates.encode(a);
            if (c)
                a = this.base62.encode(a);
            return a
        }
    }, {
        version: "3.1",
    ...
    var o = Base.extend({
        ...
        shrink: function(g) {
                    ...
                    if (b != "_no_shrink_") {
                        ...
                        for (var r = 0; r < k.length; r++) {
                            ...
                            if (!l["#" + id]) {
                                l["#" + id] = true;
                                id = rescape(id);
                                while (new RegExp(o.PREFIX + h + "\\b").test(f))
                                    h++;
            ...
            var R = new v(o.SHRUNK,function() {
                do
                    z = Packer.encode52(Q++);
                while (new RegExp("[^\\w$.]" + z + "[^\\w$:]").test(g));return z
            }
            );
            g = R.encode(g);
