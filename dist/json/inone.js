...
jsl.parser = function() {
      ...
      , d = function() {
        var a = {
            ...
            parse: function(a) {
                ...
                for (; ; ) {
                    r = c[c.length - 1],
                    this.defaultActions[r] ? s = this.defaultActions[r] : (p == null && (p = o()),
                    s = f[r] && f[r][p]);
                    if (typeof s == "undefined" || !s.length || !s[0]) {
                        if (!j) {
                            ...
                            var A = "";
                            window.errorLine = (h + 1);
                            this.lexer.showPosition ? A = "第 " + (h + 1) + " 行校验错误:\n" + this.lexer.showPosition() + "\n应该是: " + z.join(" 或 ") : A = "第 " + (h + 1) + " 行校验错误: Unexpected " + (p == 1 ? "end of input" : "'" + (this.terminals_[p] || p) + "'"),
                            this.parseError(A, {
    ...
    typeof a != "undefined" && typeof c != "undefined" && (c.parser = d,
    c.parse = function() {
    ...
    c.main = function(b) {
        ...
        if (typeof process != "undefined")
            var d = a("fs").readFileSync(a("path").join(process.cwd(), b[1]), "utf8");
        else
    ...
    function validate() {
        ...
        try {
            ...
            window.setTimeout(function() {
                $("#json_input").scrollTop((window.errorLine - 1) * 15);
            }, 600);
            $(".lineno").removeClass("bgred");
            $("#line" + window.errorLine).addClass("bgred");
        }
        $('#loadSpinner').hide()
    }
    function init() {
