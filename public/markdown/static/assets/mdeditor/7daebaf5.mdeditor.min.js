...
var inPagePreviewButtonsFullReader = _isMobile ? "in-page-preview-buttons-full-reader-x" : "in-page-preview-buttons-full-reader";
/*! zybuluo */
if (window.desktopGui) {
    var gui = window.desktopGui
      , win = gui.Window.get();
    if (gui.App.manifest.isFirstTimeStartup) {
        if (gui.App.manifest.isFirstTimeStartup = !1,
        ...
        win.show(),
        win.maximize(),
        win.on("new-win-policy", function(a, b, c) {
...
com.zybuluo.mdeditor.unifiedEditor = function() {
    function a() {
        window.lightMode || (e.renderer.setPadding(15),
        f[0].style.fontSize = "16px",
        f[0].style.lineHeight = "27px",
        f[0].style.fontFamily = 'Menlo, Ubuntu Mono, Consolas, "Courier New", Microsoft Yahei, Hiragino Sans GB, WenQuanYi Micro Hei, sans-serif')
    }
    function b(a, b) {
    ...
    var docList = [], isInitDocList = false;
    var d, e, f, g = $("#wmd-input").text();
    window.lightMode ? ($("#wmd-panel-editor").html("").append('<textarea class="wmd-input theme" id="wmd-input" spellcheck="false" autocomplete="off"></textarea>'),
    d = $("#wmd-input"),
    f = d,
    d.tabHandler()) : ($("#wmd-panel-editor").html("").append('<div style="display:none" class="wmd-input theme" id="wmd-input"></div>'),
    f = $("#wmd-input"),
    e = c()),
    ...
    var h = {
        ...
        getDefaultDocument: function() {
            ...
            if (docList.length == 0) {
                const value = localStorage.getItem('lastMarkdown_data') ?? g;
                this.addDoc(undefined, "# 新文档\n-----------\n\n");
                localStorage.removeItem('lastMarkdown_data');
                return value;
            } else {
                return localStorage.getItem(docList[0].id) ?? '';
            }
        },
        getDocList: function() {
            if (isInitDocList && docList) {
                return docList;
            }
            const str = localStorage.getItem('markdown_doclist') ?? '[]';
        ...
        addDoc: function(name, text) {
            const title = name ?? this.getTitle(text);
            const doc = {id: "markdown_" + (new Date()).getTime() + "_" + docList.length, name: title};
            docList.splice(0, 0, doc);
            localStorage.setItem(doc.id, text);
            localStorage.setItem("markdown_doclist", JSON.stringify(docList));
            this.updateDocList();
        },
        deleteDocById: function (docId) {
            if (!docId) return;
            var i = docList.findIndex((e) => e.id === docId);
            if (i < 0) return;
            this.deleteDoc(docList[i]);
        },
        deleteDoc: function(doc) {
            if (!doc) return;
            var i = docList.indexOf(doc);
            if (i < 0) return;
            docList.splice(i, 1);
            localStorage.removeItem(doc.id);
            localStorage.setItem("markdown_doclist", JSON.stringify(docList));
            this.setValue(this.getDefaultDocument());
            this.updateDocList();
        },
        ...
        updateDocList: function () {
            const a = this;
            var menus = "";
            a.getDocList().forEach((item, idx) => {
                menus = menus + '<li title="'+item.name+'"><a id="menu-doc-'+item.id+'" tabindex="-1" href="#" ><i class="icon-file-alt"></i>'+item.name+'<i class="icon-remove menu-doc-del" id="menu-doc-del-'+item.id+'" title="删除"></i></a></li>';
            });
            $("#menu-doc-items").html("").append(menus);
            a.getDocList().forEach((item) => {
                $("#menu-doc-"+item.id).on("click", function() {
                    a.openDoc(item.id);
                });
                $("#menu-doc-del-"+item.id).on("click", function() {
                    let r = com.zybuluo.common.popupConfirm.popup
                    let b = "删除文稿"
                        , c = item.name ?? "未命名"
                        , d = "删除不可撤销，确认删除【<strong>" + c + "</strong>】？"
                    r(b, d, function() {
                        $("#notification-popup-window").modal("hide");
                        a.deleteDocById(item.id);
                    }, null, null, null)
                });
            });
        },
        ...
        getCurDocName: function () {
            const list = this.getDocList();
            return list && list.length > 0 ? list[0].name : "未命名";
        },
        saveCurDoc: function () {
            const text = this.getValue();
            const title = this.getTitle(text);
            if (docList.length === 0) {
                this.addDoc(title, this.getValue());
            } else {
                localStorage.setItem(docList[0].id, this.getValue());
                if (title != docList[0].name) {
                    console.log("title change：", title);
                    docList[0].name = title;
                    localStorage.setItem("markdown_doclist", JSON.stringify(docList));
                    this.updateDocList();
                }
            }
        ...
        getTitle: function (s) {
            ...
            if (j > i) { j = 0; }
            var v = i > 0 ? s.substring(j, i) : s;
            return v.trim().substring(0, 32);
        },
        setValue: function(a) {
        ...
        applyMode: function(a) {
            window.lightMode || (e.setKeyboardHandler("vim" === a ? "ace/keyboard/vim" : "emacs" === a ? "ace/keyboard/emacs" : ""),
            e.focus())
        },
        showLineNumber: function(a) {
            window.lightMode || (e.renderer.setShowGutter(a),
            e.focus())
        },
...
com.zybuluo.mdeditor.fileManager = function() {
    function a(a) {
        o().isEditorReader && (a ? ($("#preview-published-button").show(),
        $("#preview-publish-button").hide(),
        $("#preview-published-td").show(),
        $("#preview-publish-td").hide()) : ($("#preview-publish-button").show(),
        $("#preview-published-button").hide(),
        $("#preview-publish-td").show(),
        $("#preview-published-td").hide())),
        p.isPublic = a
    }
    function b(b, c) {
        c = new Date(c).format("Y-m-d H:i"),
        parentATag = $("ul#file-list span.whiter-on-black").parent("a"),
        b === !0 ? (parentATag.attr("title", "【已发布】 " + c),
        parentATag.find("i").removeClass().addClass("icon-share-sign"),
        a(b)) : (parentATag.attr("title", "【未发布】 " + c),
        parentATag.find("i").removeClass().addClass("icon-share"),
        a(b));
            ...
            if (-1 !== a.indexOf(c)) {
                if (!h) {
                    ...
                    $(this).nextAll().each(function() {
                        return b.attr("file-created-date") > $(this).attr("file-created-date") ? (i = $(this),
                        !1) : void 0
                    }),
                ...
                h && (1 === $(this).siblings().length ? $(this).parent("ul.tag-list").parent("li").remove() : (g.parent("a").parent("li").remove(),
                e.text(Number(e.text()) - 1)))
        });
        var e = function(a) {
            var b = null;
            return $("#file-list .tag-item").each(function() {
                return a < $(this).attr("tag-name") ? (b = $(this),
                !1) : void 0
            }),
            b
        }
        ...
        r(c, d, function() {
            ...
            $.post(p.publishUserNoteUrl, {
                ...
                if (a.loggedin === !1)
                    ...
                    c.updateLocalNoteAction(p.currentId, function(b) {
                    ...
                    $("#notification-confirm").data("operation", function() {
                        $("#notification-popup-window").modal("hide"),
                        $("#notification-cancel").show()
                    })
                } else
                    a.version_error === !0 ? $("#notification-popup-window").find(".modal-body p").html('文稿已在别处被更新，请 <a href="javascript:window.location.reload();void 0;">刷新</a> 页面后重试') : a.error && ("no preview" === a.error ? ($("#notification-popup-window").find(".modal-body p").html("不能发布空白内容，请撰写内容后再发布"),
                    $("#notification-confirm").data("operation", function() {
                        $("#notification-popup-window").modal("hide")
            ...
            com.zybuluo.mdeditor.common.doUserTierFeature(a, function() {}, function() {
                $("#input-publish-password").on("focus", function() {
                    $(this).blur(),
    ...
    function i() {
        s.get(function(a) {
            return null === a ? void t.warningToSaveBeforeLogin(function() {
                ...
                c.checkLocalNoteAction(p.currentId, function(a) {
                ...
                r(a, d, function() {
                    ...
                    $.post(p.publishUserNoteUrl, {
                        ...
                        if (a.loggedin === !1)
                            ...
                            a.version_error === !0 ? $("#notification-popup-window").find(".modal-body p").html('文稿已在别处被更新，请 <a href="javascript:window.location.reload();void 0;">刷新</a> 页面后重试') : a.error && $("#notification-popup-window").find(".modal-body p").html('出错啦，请 <a href="javascript:window.location.reload();void 0;">刷新</a> 页面后重试')
                    }, "json")
                }, null, null, function() {
                    t.focus()
                })
        ...
        com.zybuluo.mdeditor.common.isOnline(function() {
            ...
            r(a, b, function() {
                ...
                $.post(p.removePasswordUserNoteUrl, {
                    ...
                    a.loggedin === !1 ? ($("#notification-popup-window").modal("hide"),
    ...
    function m(a) {
        s.get(function(b) {
            ...
            r(c, d, null, null, function() {
                $("#notification-popup-window #notification-confirm").hide();
                var c = !1;
                com.zybuluo.mdeditor.common.doUserTierFeature(b, function() {
                    $("#cb-offline-local-file").on("click", function() {
                    ...
                    $("#cb-offline-local-file").on("click", function() {
                        ...
                        a ? window.open(p.paymentUrl) : (toastr.clear(),
                  ...
                  , e = function(a, c) {
                    ...
                    e.onload = function(a) {
                        return function(e) {
                            ...
                            return a.type.match("text.*") || "" === a.type && !f.match(/\uFFFD/) ? 0 === a.size ? void d.show().html("<strong>警告！</strong>文件【" + a.name + "】内容为空，请重新选择文件。") : a.size > 102400 ? void d.show().html("<strong>警告！</strong>文件【" + a.name + "】内容大于 100KB，请重新选择文件。") : (d.hide(),
                            void v(b, c, f)) : void d.show().html("<strong>警告！</strong>文件【" + a.name + "】不是文本文件，请重新选择文件。")
                        }
                    }(a);
                    var f = a.slice(0, 102400);
                    e.readAsText(f)
                };
                document.getElementById("input-file-open-local-file").addEventListener("change", function(a) {
    ...
    function n() {
        s.get(function(a) {
            ...
            r(b, c, function() {
                ...
                $.post(p.noteRevisionSaveUrl, {
                    ...
                    if (a.loggedin === !1)
                        ...
                        if (a.is_saved === !0) {
                            ...
                            b.updateLocalNoteAction(p.currentId, function(b) {
                                p.currentVersionId = a.version_id,
                                b.version_id = a.version_id
                            }, null, function() {
                                p.currentVersionId = a.version_id
                            ...
                            $("#notification-popup-window").find(".modal-body p").html('当前文稿和最近的历史版本内容相同，不再重复保存，您可以前往工具栏【<i class="icon-th-large"></i>文稿信息】 菜单查看，或使用快捷键 Ctrl+Alt+I 查看。');
                        $("#notification-cancel").hide(),
                        $("#notification-confirm").data("operation", function() {
                            $("#notification-popup-window").modal("hide"),
                            $("#notification-cancel").show()
                ...
                p.currentId < 0 && ($("#notification-popup-window").find(".modal-body p").html("无法手动保存离线文稿的历史版本。"),
    ...
    var o = com.zybuluo.mdeditor.common.getCurrentMode
      ...
      , v = function(a, b, c) {
        var d = function(a, b, c, d) {
            ...
            if (null !== d && "" !== d && (j.local_file_path = d,
            void 0 === b)) {
                var k = require("fs");
                try {
            ...
            e.createLocalNoteAction(j, c, function() {
        ...
        if (null !== b)
            return $("#notification-popup-window").modal("hide"),
        ...
        $.post(p.newUserNoteUrl, e, function(a) {
            a.loggedin === !1 ? ($("#notification-popup-window").modal("hide"),
            window.location = p.loginComeFromUrl) : a.userNoteId && a.version_id ? ($("#notification-popup-window").modal("hide"),
            $.localStorage("currentId", a.userNoteId),
            window.location.hash = "#" + a.userNoteId,
            window.location.reload()) : a.error && $("#notification-popup-window").find(".modal-body p").html('出错啦，请 <a href="javascript:window.location.reload();void 0;">刷新</a> 页面后重试')
        }, "json").fail(function() {
            $("#notification-popup-window").modal("hide"),
            d(a, c, "create", null)
        })
      ...
      , y = function(a, b) {
        ...
        var g = [];
        d.find(".flow-diagram:has(>svg), .sequence-diagram:has(>svg), div.mermaid:has(>svg)").each(function() {
            g.push($(this))
        }),
        z(g, 0, function() {
      ...
      , A = function(a) {
        ...
        c.children("div.mermaid").removeAttr("data-processed"),
        ...
        var f = function(a, b) {
            $childULElement = a.siblings("ul"),
            0 !== $childULElement.length && g($childULElement);
            var c = a.parent("li.todo-list-item");
            a.replaceWith(b === !1 ? '<img src="' + p.staticImgUrl + 'icon-check-empty.png" style="margin: 0 6px 0 -20px; vertical-align: middle;"></img>' : '<img src="' + p.staticImgUrl + 'icon-check-sign.png" style="margin: 0 6px 0 -20px; vertical-align: middle;"></img>'),
            c.replaceWith('<div class="todo-list-item">' + c.html() + "</div>")
        }
          , g = function(a) {
            a.find("li.todo-list-item > i.icon-check-empty").each(function() {
                f($(this), !1)
            }),
            a.find("li.todo-list-item > i.icon-check-sign").each(function() {
                f($(this), !0)
            }),
            a.find("div.todo-list-item").each(function() {
                var a = $(this).parent("ul");
                a.replaceWith('<div style="margin: 0 0 0 25px">' + a.html() + "</div>")
            })
        };
        g(c);
        var h = [];
        ...
        c.find("svg").each(function(a) {
            var b = $(this);
            void 0 === b.attr("width") && b.attr("width", h[a].width),
            void 0 === b.attr("height") && b.attr("height", h[a].height),
            i.push(b.parent())
        }),
        z(i, 0, function() {
      ...
      , C = function(a, b, c) {
        var docList = com.zybuluo.mdeditor.unifiedEditor.getDocList()
        var d = null
          , e = null
          , f = docList.length > 0 ? docList[0].name : 'unknow';
        if ("markdown" === b)
            q(c, f + ".md");
        else if ("markdowns" === b) {
            var g = new JSZip
              , h = com.zybuluo.mdeditor.syncUserNotes;
            h.exportAllLocalNotes(a, function(a) {
                for (var b in a.tags) {
            ...
            d = "<!DOCTYPE html>\n<html" + j + '>\n<head>\n<meta charset="utf-8">\n' + 
                ...
                '<div id="wmd-preview" class="wmd-preview wmd-preview-full-reader">' + $("#wmd-preview").html() + "</div>\n</body>\n</html>",
            q(d, f + ".html")
        } else if ("print" === b) {
            var i = com.zybuluo.mdeditor.common.getCurrentThemeClassName()
                , j = ' class="theme ' + i + '"';
...
pre.prettyprint {
    display: block; 
    box-shadow: 0 1px 2px rgba(255, 255, 255, 0.1) inset, 45px 0 0 rgba(102, 128, 153, 0.05) inset, 0 1px 0 rgba(102, 128, 153, 0.05)!important;
    padding: 4px 4px 4px 9.5px!important;
}
...
.wmd-preview .custom-block>p, li>p {text-indent:0em!important; padding-left: 0!important}

.wmd-preview .custom-block.warning {
    border-color: #e7c000;
    color: #6b5900!important;
}
.wmd-preview .custom-block.danger {
    border-color: #c00;
    color: #4d0000!important;
}

.wmd-preview .custom-block.details {
    border: none;
...
.wmd-preview .custom-block.details .summary {
    cursor: pointer;
    display: list-item;
    outline: none;
...
.wmd-preview .mark {
    padding: .1em;
    border: 1px solid #f8f840;
    background-color: #f8f840!important;
}

.wmd-preview .code-header {
    display: none;
}
...
ul.todo-list-item, ol.todo-list-item {
    padding: 0;
...
li.todo-list-item i {
    margin: 0 0.55em 0 -1.35em;
    vertical-align: middle;
}

@font-face {
    font-family: FontAwesome;
    src: url(https://www.zybuluo.com/static/font/fontawesome-webfont.eot?v=3.2.1);
    src: url(https://www.zybuluo.com/static/font/fontawesome-webfont.eot?#iefix&v=3.2.1) format('embedded-opentype'),
                url(https://www.zybuluo.com/static/font/fontawesome-webfont.ttf?v=3.2.1) format('truetype'),
                url(https://www.zybuluo.com/static/font/fontawesome-webfont.svg#fontawesomeregular?v=3.2.1) format('svg');
...
.linenums ol, .linenums ul {
  line-height: 1.35em;
  font-size: 0.85em;
}
pre, blockquote, svg, h4, h5, h6, table, dl { margin: 0!important; }
blockquote>p, li>p {text-indent:0em!important; padding-left: 0!important}

.clearfix:after{display:block;visibility:hidden;clear:both;height:0;content:" ";font-size:0}.icons-ul{margin-left:2.142857142857143em;list-style-type:none}.icons-ul>li{position:relative}.icons-ul.icon-li{position:absolute;left:-2.142857142857143em;width:2.142857142857143em;text-align:center;line-height:inherit}[class^=icon-].hide,[class*=" icon-"].hide{display:none}.icon-muted{color:#eee}.icon-light{color:#fff}.icon-dark{color:#333}.icon-border{border:solid 1px#eee;padding:.2em.25em.15em;-webkit-border-radius:3px;-moz-border-radius:3px;border-radius:3px}.icon-2x{font-size:2em}.icon-2x.icon-border{border-width:2px;-webkit-border-radius:4px;-moz-border-radius:4px;border-radius:4px}.icon-3x{font-size:3em}.icon-3x.icon-border{border-width:3px;-webkit-border-radius:5px;-moz-border-radius:5px;border-radius:5px}.icon-4x{font-size:4em}.icon-4x.icon-border{border-width:4px;-webkit-border-radius:6px;-moz-border-radius:6px;border-radius:6px}.icon-5x{font-size:5em}.icon-5x.icon-border{border-width:5px;-webkit-border-radius:7px;-moz-border-radius:7px;border-radius:7px}.pull-right{float:right}.pull-left{float:left}[class^=icon-].pull-left,[class*=" icon-"].pull-left{margin-right:.3em}[class^=icon-].pull-right,[class*=" icon-"].pull-right{margin-left:.3em}
...
a[class^=icon-],a[class*=" icon-"]{display:inline}.icon-glass:before{content:"\\f000"}.icon-glass:before{content:"\\f000"}.icon-music:before{content:"\\f001"}.icon-search:before{content:"\\f002"}.icon-envelope-alt:before{content:"\\f003"}.icon-heart:before{content:"\\f004"}.icon-star:before{content:"\\f005"}.icon-star-empty:before{content:"\\f006"}.icon-user:before{content:"\\f007"}.icon-film:before{content:"\\f008"}.icon-th-large:before{content:"\\f009"}.icon-th:before{content:"\\f00a"}.icon-th-list:before{content:"\\f00b"}.icon-ok:before{content:"\\f00c"}.icon-remove:before{content:"\\f00d"}.icon-zoom-in:before{content:"\\f00e"}.icon-zoom-out:before{content:"\\f010"}.icon-power-off:before,.icon-off:before{content:"\\f011"}.icon-signal:before{content:"\\f012"}.icon-gear:before,.icon-cog:before{content:"\\f013"}.icon-trash:before{content:"\\f014"}.icon-home:before{content:"\\f015"}.icon-file-alt:before{content:"\\f016"}.icon-time:before{content:"\\f017"}.icon-road:before{content:"\\f018"}.icon-download-alt:before{content:"\\f019"}.icon-download:before{content:"\\f01a"}.icon-upload:before{content:"\\f01b"}.icon-inbox:before{content:"\\f01c"}.icon-play-circle:before{content:"\\f01d"}.icon-rotate-right:before,.icon-repeat:before{content:"\\f01e"}.icon-refresh:before{content:"\\f021"}.icon-list-alt:before{content:"\\f022"}.icon-lock:before{content:"\\f023"}.icon-flag:before{content:"\\f024"}.icon-headphones:before{content:"\\f025"}.icon-volume-off:before{content:"\\f026"}.icon-volume-down:before{content:"\\f027"}.icon-volume-up:before{content:"\\f028"}.icon-qrcode:before{content:"\\f029"}.icon-barcode:before{content:"\\f02a"}
.icon-tag:before{content:"\\f02b"}.icon-tags:before{content:"\\f02c"}.icon-book:before{content:"\\f02d"}.icon-bookmark:before{content:"\\f02e"}.icon-print:before{content:"\\f02f"}.icon-camera:before{content:"\\f030"}.icon-font:before{content:"\\f031"}.icon-bold:before{content:"\\f032"}.icon-italic:before{content:"\\f033"}.icon-delete:before{content:"\\f0cc"}.icon-center:before{content:"\\f037"}.icon-checklist:before{content:"\\f046"}.icon-text-height:before{content:"\\f034"}.icon-text-width:before{content:"\\f035"}.icon-align-left:before{content:"\\f036"}.icon-align-center:before{content:"\\f037"}.icon-align-right:before{content:"\\f038"}.icon-align-justify:before{content:"\\f039"}.icon-list:before{content:"\\f03a"}.icon-indent-left:before{content:"\\f03b"}.icon-indent-right:before{content:"\\f03c"}.icon-facetime-video:before{content:"\\f03d"}.icon-picture:before{content:"\\f03e"}.icon-pencil:before{content:"\\f040"}.icon-map-marker:before{content:"\\f041"}.icon-adjust:before{content:"\\f042"}.icon-tint:before{content:"\\f043"}.icon-edit:before{content:"\\f044"}.icon-share:before{content:"\\f045"}.icon-check:before{content:"\\f046"}.icon-move:before{content:"\\f047"}.icon-step-backward:before{content:"\\f048"}.icon-fast-backward:before{content:"\\f049"}.icon-backward:before{content:"\\f04a"}.icon-play:before{content:"\\f04b"}.icon-pause:before{content:"\\f04c"}.icon-stop:before{content:"\\f04d"}.icon-forward:before{content:"\\f04e"}.icon-fast-forward:before{content:"\\f050"}.icon-step-forward:before{content:"\\f051"}.icon-eject:before{content:"\\f052"}.icon-chevron-left:before{content:"\\f053"}
...
`;
            ...
            iframe.onload = function() {
                ...
                setTimeout(function() {document.body.removeChild(iframe)}, 120);
            }
        } else
            "pdf" === b && com.zybuluo.mdeditor.common.isOnline(function() {
                y(f, function(a) {
                    var b = "导出 PDF 文件"
                    ...
                    r(b, c, function() {
                        ...
                        b.onreadystatechange = function() {
                            if (4 == this.readyState)
                                if (201 == this.status)
                                    pdf = this.response,
                                    void 0 !== pdf && ($("#notification-popup-window").modal("hide"),
                                    q(pdf, f + ".pdf"));
                                    ...
                                    a.onloadend = function() {
                                        var b = a.result
                                          ...
                                          , d = JSON.parse(c);
                                        d.loggedin === !1 ? ($("#notification-popup-window").modal("hide"),
                                        window.location = p.loginComeFromUrl) : d.version_error === !0 ? $("#notification-popup-window").find(".modal-body p").html('文稿已在别处被更新，请 <a href="javascript:window.location.reload();void 0;">刷新</a> 页面后重试') : d.error && $("#notification-popup-window").find(".modal-body p").html('出错啦，请 <a href="javascript:window.location.reload();void 0;">刷新</a> 页面后重试')
                                    }
                                } else
                                    $("#notification-popup-window").find(".modal-body p").html("导出 PDF 文件失败，服务器可能正忙，请稍后再试，或联系我们")
                        }
                        ,
      ...
      , E = function() {
        var a = null
          , b = function(b, c) {
            a = b.watch(c, function() {
                window.confirm('正在编辑的文件 "' + c + '" 有变动，是否重新加载？') && (a.close(),
                window.location.reload())
            })
        }
          , c = function() {
...
com.zybuluo.mdeditor.scrollLink = function() {
    var a, b = com.zybuluo.mdeditor.unifiedEditor, c = {
    ...
    ;
    var f, g, h, i = [], j = [], k = -10, l = -10, m = _.debounce(function(b) {
        function c(a) {
            var b = r;
            if (void 0 !== a) {
        ...
        s()
    }, 500), n = !1, o = !1, p = !1, q = !1, r = $("<div>"), s = _.throttle(function() {
        function a(a, b, c) {
        ...
        if (0 === i.length || i.length !== j.length)
            return void s();
        ...
        if (n === !0) {
            ...
            k = b,
            c = a(b, i, j),
            c = _.min([c, h.prop("scrollHeight") - h.outerHeight()]),
            Math.abs(c - e) <= 9)
                return void (l = e);
            ...
            l = e,
            c = a(e, j, i),
            window.lightMode ? c = _.min([c, f.prop("scrollHeight") - f.outerHeight()]) : (c = _.min([c, d.session.getScreenLength() * d.renderer.lineHeight + d.renderer.scrollMargin.bottom - d.renderer.$size.scrollerHeight]),
            0 > c && (c = 0)),
            Math.abs(c - b) <= 9)
                return void (k = b);
            r.stop("scrollLinkFx", !0).css("value", 0).animate({
    ...
    c.onEditorConfigure = function(a) {
        u = $("#wmd-preview"),
        a.getConverter().hooks.chain("postConversion", function(a) {
...
com.zybuluo.mdeditor.mode = function() {
    function a() {
        if (f().isFullReader && 0 === $editorReaderFull.has("#wmd-preview").length) {
            ...
            var a = $("#reader-full-topInfo").clone().removeClass("reader-full-topInfo-hidden").addClass(_isMobile ? "reader-full-topInfo-shown-x" : "reader-full-topInfo-shown");
            $editorReaderFull.append(a),
            $editorReaderFull.append($(".in-page-preview-buttons").addClass(inPagePreviewButtonsFullReader)),
    ...
    var j = function() {
        ...
        n(),
        window.lightMode ? (i.css("max-width", "850px").focus(),
        $("#md-section-helper").attr("style", "max-width:850px; overflow:hidden; word-wrap:break-word; resize: none;")) : b.focus(),
        $(window).unbind("resize");
        var e = null;
        e = window.lightMode ? function() {
            i.autosize(),
      ...
      , k = function(a) {
        ...
        if (c) window.location.hash = null !== c ? c : "";
        ...
        n(),
        window.lightMode ? ($("#md-section-helper").removeAttr("style"),
        i.trigger("autosize.destroy"),
        i.removeAttr("style")) : (i.removeAttr("style"),
        b.restoreAceEditor(),
        b.getEditor().resize()),
      ...
      , l = function(b) {
        ...
        $("#container").hide(),
        $editorReaderFull.removeClass("editor-reader-full-hidden").addClass("editor-reader-full-shown"),
        $editorReaderFull.css("position", "static").css("padding-right", isMobile() ? "25px" : "75px"),
        $("#preview-reader-small-button").show(),
        $("#preview-reader-expand2-button").show(),
      ...
      , m = function(a) {
        ...
        if (b) window.location.hash = null !== b ? b : "";
        ...
        $("#wmd-panel-preview").before($(".in-page-preview-buttons").removeClass(inPagePreviewButtonsFullReader)),
        ...
        $(".in-page-preview-buttons").removeClass(inPagePreviewButtonsFullReader),
        $("#preview-reader-small-button").hide(),
        $("#preview-reader-expand2-button").hide(),
      ...
      , q = function() {
        var a = !1
          , b = function() {
            a ? ($("#wmd-button-bar").show(),
            $("#preview-button-bar").show(),
            a = !1) : ($("#wmd-button-bar").hide(),
            $("#preview-button-bar").hide(),
            a = !0)
        };
        return {
            hideToolBar: b,
      ...
      , r = function() {
        var a = !1
          , b = function() {
            a ? ($("#editor-column").removeClass("pull-right").addClass("pull-left"),
            $("#preview-column").removeClass("pull-left").addClass("pull-right"),
            $("#preview-button-row").removeClass("pull-left").addClass("pull-right"),
            $("ul.dropdown-menu.pull-left").removeClass("pull-left").addClass("pull-right"),
            $("#editor-reader-exchange-button span").removeClass("icon-chevron-sign-right").addClass("icon-chevron-sign-left"),
            a = !1) : ($("#editor-column").removeClass("pull-left").addClass("pull-right"),
            $("#preview-column").removeClass("pull-right").addClass("pull-left"),
            $("#preview-button-row").removeClass("pull-right").addClass("pull-left"),
            $("ul.dropdown-menu.pull-right").removeClass("pull-right").addClass("pull-left"),
            $("#editor-reader-exchange-button span").removeClass("icon-chevron-sign-left").addClass("icon-chevron-sign-right"),
            a = !0),
    ...
    return {
        ...
        switchExpandReaderMode: () => {
            var isExpand = $("#wmd-preview").hasClass("wmd-preview-expand");
            isExpand ? $("#wmd-preview").removeClass("wmd-preview-expand") : $("#wmd-preview").addClass("wmd-preview-expand");
            localStorage.setItem("review_isExpand", !isExpand);
        },
...
com.zybuluo.mdeditor.syncUserNotes = function() {
    var a = com.zybuluo.indexedDB
      ...
      , e = function(c, e, g, h, i) {
        var j = a.getDatabase();
        if (null === c || d.isUrlMode === !0 || null === j)
            return void (i && i());
        g && g();
        ...
        n.openCursor(o).onsuccess = function(a) {
            ...
            if (b) {
                ...
                $.post(d.mdeditorNoteSyncUrl, {
                    local_username: c.name,
                    current_id: e,
                    local_note_action_map: JSON.stringify(l)
                }, function(a) {
                    if (a.loggedin === !1)
                        window.location = d.loginComeFromUrl;
                    else if (a.synchronized_usernote_actions) {
                        var b = JSON.parse(a.synchronized_usernote_actions)
                          , c = a.sync_left_over;
                        f(b, l, c, h, i)
                    } else
                        a.version_error === !0 ? (console.error("synchronize notes: version error"),
                        i("同步失败：遭遇版本冲突，请稍后重试或联系我们。")) : a.error && (console.error("synchronize notes: error"),
                        i("同步失败：非法请求，请退出重登或联系我们。"))
                }, "json").fail(function() {
      ...
      , f = function(c, d, e, f, g) {
        ...
        for (var n in c) {
            var o = c[n];
            "create" === o.action || "modify" === o.action ? o.server_data === !1 && (localNoteAction = d[o.local_note_id],
            m(localNoteAction, o)) : "merge" === o.action && i.push(o.server_note_id),
            h(l, o)
        }
    }
      , g = function(c, d, e) {
      ...
      , h = function(a, b) {
        ...
        } else if ("delete" === b.action)
            a.delete(b.local_note_id);
        else if ("modify" === b.action)
            i(a, b);
        ...
        var i = h.objectStore(b)
          ...
          , k = function() {
            ...
            a.onsuccess = function(a) {
                return a.target.result ? (j -= 1,
                void k()) : (c.local_note_id = j,
                c.action = d,
                c.base = c.details,
                void i.add(c))
            }
    ...
    var b = com.zybuluo.mdeditor.initData
      ...
      , n = function(a, c, d, e, f, g) {
        if (a) {
            ...
            a.user_tier ? (i.html('<span class="paid-user-color">' + a.name + "</span>"),
            $("#mdeditor-payment-submenu").removeClass("paid-user-color").children("span").text("高级会员")) : i.html("<span>" + a.name + "</span>"),
            h.removeClass("editor-reader-hidden")
        } else
            $("#preview-button-row-notlogin.editor-reader-hidden").removeClass("editor-reader-hidden");
        b.isUrlMode === !1 && ($("#preview-published-button.editor-reader-hidden").removeClass("editor-reader-hidden"),
        $("#preview-publish-button.editor-reader-hidden").removeClass("editor-reader-hidden"),
        $("#preview-file-button.editor-reader-hidden").removeClass("editor-reader-hidden"),
        $("#preview-file-button .btn-group").show(),
        $("#preview-button-row-spacer.editor-reader-hidden").removeClass("editor-reader-hidden"),
        $("#preview-list-button.editor-reader-hidden").removeClass("editor-reader-hidden"),
        $("#preview-info-button.editor-reader-hidden").removeClass("editor-reader-hidden"),
        $("#preview-doc-button.editor-reader-hidden").removeClass("editor-reader-hidden"),
        $("#preview-settings-button.editor-reader-hidden").removeClass("editor-reader-hidden")),
      ...
      , o = function() {
        ...
        if (a)
            ...
            null !== f && (c = f),
            e += c,
            c ? i.checkLocalNoteAction(c, function(a) {
                a ? p(e, a) : p(e, null)
            }, function() {
                p(e, null)
            }) : p(e, null)
      ...
      , p = function(a, e) {
        ...
        $.post(a, {
            ...
            if (a.version_error === !0)
                return window.alert("遭遇数据版本冲突，点击【确认】后自动重试。"),
            ...
            b.isUrlMode === !0 && (window.alert("离线状态下无法开启外链接模式，请连接网络后重试，点击确认回到离线模式。"),
            ...
            d.get(function(a) {
                ...
                b.isUrlMode = !1;
                var g = 0;
                i.populateOfflineData(a, e, function(b, c) {
                    var d = null;
                    if (null !== b)
                        d = {
                        ...
                        i.createLocalNoteAction(b, "create", function() {
      ...
      , r = function(a, c, d, e) {
        if (null === h)
            return void e();
        if (null === d.action)
            return null === a || b.isUrlMode === !0 ? void e() : d.server_data === !0 ? void i.checkLocalNoteAction(d.server_note_id, function(a) {
                a ? com.zybuluo.mdeditor.common.reloadCurrentId(d.server_note_id) : (d.action = "create",
                i.syncCurrentServerNoteAction(d, e))
            }, function() {
                window.alert("检测本地存储时出错，请关闭客户端或浏览器后重启程序，或者联系我们。")
            }) : (i.copyLocalNoteActionToServerNoteAction(c, d),
            void e());
        if ("offline" === d.action)
            return i.copyLocalNoteActionToServerNoteAction(c, d),
            void e();
        if ("create" === d.action)
            return d.server_data === !0 ? void i.syncCurrentServerNoteAction(d, e) : (i.copyLocalNoteActionToServerNoteAction(c, d),
            void i.syncCurrentServerNoteAction(d, e));
        if ("modify" === d.action)
            return d.server_data === !0 ? void i.syncCurrentServerNoteAction(d, e) : (i.copyLocalNoteActionToServerNoteAction(c, d),
            void i.syncCurrentServerNoteAction(d, e));
        if ("merge" === d.action) {
      ...
      , s = function(d, e, g, h) {
        ...
        b.isEditablePage === !0,
        null !== d && null !== e.title && $("title").first().text(e.title + " - Markdown 编辑阅读器");
        var l = function() {
        ...
        if (window.desktopGui && e.local_file_path) {
            ...
            try {
                ...
                if (p !== e.details) {
                    ...
                    i.updateLocalNoteAction(j, function(a) {
                        a.details = p,
                ...
                $("title").first().text(t + " (" + u + ") - Markdown 编辑阅读器")
            } catch (v) {
                console.error(v),
...
com.zybuluo.mdeditor.MergeEditor = function(a, b, c) {
    ...
    var h = e.diff_prettyHtml(g)
      , i = function(a, b, c) {
        ...
        e.after(i + j),
        e.get(0).addEventListener("paste", function(a) {
            a.preventDefault();
            var b = null
              , c = document.createElement("div");
            if (a.clipboardData) {
                if (b = (a.originalEvent || a).clipboardData.getData("text/plain"),
                "" === b)
                    return !1;
        ...
        k(l),
        e.focus(),
        $("#merge-editor > span").on("mousedown", function() {
            l = $(this)
        }),
        $("#merge-editor > span").on("keydown", function() {
            l = $(this)
        }),
        $("#previous-diff-button").on("click", function() {
        ...
        $("#next-diff-button").on("click", function() {
...
com.zybuluo.mdeditor.init = function(a) {
    function b(a) {
        ...
        return b && (E = a,
        l.reset_tag_changes(E),
        I === !1 && (F === !1 ? (F = !0,
        $("#file-list-topbar").css("display", "block"),
        $("#file-list").css("display", "block"),
        $("#search-file-bar").css("display", "none"),
        $("#tag-file-bar").css("display", "block")) : clearTimeout(G),
        $("#file-list .tag-item:visible").hide(),
        $("#file-list .file-item:visible").hide(),
        $("#file-list .whiter-on-black").parent("a").parent("li").siblings(".tag-item").show(),
    ...
    function c(a) {
        var b, c, d, e, f = a.match(/^@@\s(.*)\s@@/), g = f[1].split(" "), h = g[0].match(/^-(\d+),(\d+)/);
        h ? (b = h[1],
        c = h[2]) : (b = g[0].match(/^-(\d+)/)[1],
        c = 1);
        var i = g[1].match(/^\+(\d+),(\d+)/);
        return i ? (d = i[1],
        e = i[2]) : (d = g[1].match(/^\+(\d+)/)[1],
        e = 1),
        [b, c, d, e]
    }
    function d(a) {
        for (var b = q(4); -1 !== a.indexOf(b); )
            b = q(4);
        return a.push(b),
        b
    }
    function e(a, b) {
        var c = bb[b];
        return null !== c ? (a.push(Z[c]),
        Z[c]) : null
    }
    function f(a) {
        var b = Z.join(" ");
        b !== a && (n.anchorListString = a,
        Z = null,
        ab = null,
        bb = null)
    }
    function g(a) {
        ...
        if (null === Z && null === ab) {
            if (null !== n.anchorListString) {
                Z = "" === n.anchorListString ? [] : n.anchorListString.split(" "),
                ab = [],
                bb = [];
                var h = 0;
        ...
        for (var j = [], k = [], l = u(function(a, b) {
            j.push(a.outerHTML),
            k.push(b ? i++ : null)
        }, g), m = difflib.unifiedDiff(ab, j), o = [], p = 0, q = 2; q < m.length; ) {
            var r = c(m[q])
              , s = r[0];
            for (r[1],
            r[2],
            r[3]; s - 1 > p; p++)
                e(o, p);
            for (var t = -1, v = 0, w = 0; ++q < m.length; ) {
                var x = m[q][0];
                if (" " === x) {
                    if (v > 0) {
                        for (w = 0; v > w; w++)
        ...
        bb = k,
        Z = o,
        l !== Z.length && console.error("The number of generated anchor ids are different from the qualified elements."),
        {
            anchorIdsContainer: Z,
            wmdPreviewDiffResults: b,
            overwrite: !1
    ...
    function h(a, b, c, d, e) {
        ...
        x(a, b, function() {
            ...
            $("#notification-popup-window .modal-body"));
    ...
    Markdown.Extra.init(A, {
        extensions: ["tables", "fenced_code_gfm", "def_list", "attr_list", "footnotes", "smartypants", "strikethrough", "highlightText", "iconfonts", "newlines"],
    ...
    var D = /((\n|^)(标签|tags)(.*?)(:|：)[^\S\n]*(\S*.*?)(\n|$))/i
      ...
      , J = !1
      , K = [];
    A.hooks.chain("preConversion", function(c) {
        if (a) {
            var d = []
    ...
    var L = !1
      , M = function(a, b, c) {
        ...
        var d = m(a, b, !0)
          , e = !1;
        x("人工合并文稿版本", "此文稿在本地和云端有过不同的修改，请编辑以下高亮部分确保文稿的正确性。编辑完成后点击<strong>【完成编辑】</strong>， 或者点击<strong>【使用本地版本】</strong>继续使用本地版本。<br><br>" + d.html(), function() {
            e = !0,
            $("#notification-popup-window").modal("hide");
            var g = d.text();
            N(b, g),
            ...
            var b = 600
              , c = 400;
            $(window).height() <= 720 && (a.css("top", "0px"),
            b = $(window).height(),
            c = b - 250),
      ...
      , P = function(a) {
        if (!a)
            return delete n.mergeServerNoteAction,
            {
                isMerge: !1
            };
        ...
        if (b) {
            if (O === !0) {
                ...
                if (d[0] !== a.details) {
                    ...
                    for (var g in d[1])
                        d[1][g] === !1 && (e = !0);
                    return e ? (M(b.details, a.details, b.anchor_list),
                    null) : (N(a.details, d[0]),
                    d[0] === b.details && f(b.anchor_list),
                    L = !0,
                    B.refreshPreview(),
                    null)
                }
      ...
      , S = function(b, c) {
        if (n.isEditablePage === !0) {
            ...
            if (e && a) {
                ...
                g.text() !== e && (g.text(e),
                $("title").first().text(e + " - Markdown 编辑阅读器"))
            }
            var h = function(f) {
                var g = P(f);
                if (null === g)
                ...
                if (g.isMerge === !0)
                    I = !1,
                    $("#wmd-editor-save-button").show(),
                    $("#wmd-editor-save-button > span").text("合并中..."),
                    h = {
                        title: f.title,
                ...
                I === !0)
                    ...
                    k = w.patch_toText(k),
                    $("#wmd-editor-save-button").show(),
                    $("#wmd-editor-save-button > span").text("保存中..."),
                    h = null !== c ? {
                        title: e,
                        diff_details: k,
                        tags: JSON.stringify(b),
                ...
                $.ajax({
                    ...
                    eb = !1,
                    b.loggedin === !1 ? window.location = n.loginComeFromUrl : b.id && b.version_id ? (a ? (Q = !0,
                    l.reset_file_public_changes(b.is_public, b.updated_date),
                    v.updateLocalNoteAction(n.currentId, function(a) {
                        n.currentVersionId = b.version_id,
                        g.isMerge === !1 ? (a.action = null,
                        a.base = d,
                        a.last_updated_date = b.updated_date,
                        a.version_id = b.version_id,
                        a.public = b.is_public) : (a.base = d,
                        a.last_updated_date = b.updated_date,
                        a.version_id = b.version_id,
                        a.public = b.is_public,
                        a.details === d ? a.action = null : (a.action = "modify",
                        L = !0,
                        B.refreshPreview()))
                    }, null, function() {
                        n.currentVersionId = b.version_id
                    }),
                    f || (R = d)) : n.currentVersionId = b.version_id,
                    H = !1,
                    gb = !1,
                    g.isMerge === !1 ? ($("#wmd-editor-save-button > span").text("已保存"),
                    $("#wmd-editor-save-button").delay(3e3).fadeOut()) : ($("#wmd-editor-save-button > span").text("已合并"),
                    $("#wmd-editor-save-button").delay(3e3).fadeOut())) : b.version_error === !0 && b.version_id ? f ? g.isMerge === !1 ? (n.mergeServerNoteAction = {
                        details: b.details,
                        version_id: b.version_id,
                        anchor_list: b.anchor_list
                    },
                    L = !0,
                    B.refreshPreview()) : (n.mergeServerNoteAction = {
                        details: b.details,
                        version_id: b.version_id,
                        anchor_list: b.anchor_list
                    },
                    L = !0,
                    B.refreshPreview()) : (window.alert("当前文稿在别处有新的更新，点击【确认】后重新加载。"),
                    window.location.reload()) : b.error && (window.alert("保存文稿时遇到问题，点击【确认】后重试。"),
                    window.location.reload())
                }).fail(function() {
                    eb = !1,
                    $("#wmd-editor-save-button > span").css("color", "#FFFFAA").text("离线模式"),
                    H = !0,
                    gb = !1,
            ...
            a ? v.updateLocalNoteAction(n.currentId, function(a) {
                n.currentVersionId < a.version_id || (void 0 !== n.mergeServerNoteAction && (a.action = "merge"),
                null === a.action && (a.action = "modify"),
                e && (a.title = e),
                a.details = d,
                0 !== b.length && (a.tags = b),
                a.anchor_list = c,
                a.last_updated_date = i)
            }, function(b) {
                if (n.currentVersionId < b.version_id)
                    Q === !0 && window.alert("当前文稿在别处已被同步，点击【确认】后重新从本地加载。"),
                    n.currentVersionId = b.version_id,
                    n.isPublic = b.public,
                    N(d, b.details),
                    f(b.anchor_list),
                    L = !0,
                    B.refreshPreview();
                else if ("offline" !== b.action)
                    J = !1,
                    h(b);
                else {
                    var c = $("#wmd-editor-save-button");
                    if (c.is(":visible") || (c.attr("title", "离线文稿只存储在本机，任何情况下不与云端同步，企业级私密").show(),
                    $("#wmd-editor-save-button > span").css("color", "#FFFFAA").text("离线文稿")),
                    J = !0,
                    I === !0 && (I = !1),
                    H = !0,
                    window.desktopGui && b.local_file_path) {
                        var e = require("path")
                          , g = e.basename(b.local_file_path)
                          , j = e.dirname(b.local_file_path);
                        $("title").first().text(g + " (" + j + ") -  Markdown 编辑阅读器");
                        var k = require("fs")
                          , m = l.fileWatcher;
                        m.stop(),
                        k.writeFile(b.local_file_path, b.details, function(a) {
                            return gb = !1,
                            m.watch(k, b.local_file_path),
                            a ? console.error(a) : void console.log("The file " + b.local_file_path + " was saved!")
                        })
                    } else
                        gb = !1;
                    a && l.reset_file_public_changes(n.isPublic, i)
      ...
      , V = !0;
    A.hooks.chain("postConversion", function(b) {
        var c = null
          , d = null
          , e = g(b);
        ...
        for (var h in d) {
            var i = d[h]
              , j = i.index + f;
            if ("add" === i.action)
                0 === j ? (z.prepend(i.wmdPreviewChildNode),
                i.anchorId && z.children()[0].setAttribute("data-anchor-id", i.anchorId)) : ($(z.children()[j - 1]).after(i.wmdPreviewChildNode),
    ...
    var W = 0
      , X = function(a, b) {
        ...
        try {
            var d = null;
            "flow" === b ? (d = flowchart.parse(a),
            d.drawSVG("uml-X-x-X-diagram", {
                "line-width": 2,
                "line-length": 50,
                "text-margin": 10,
                "font-size": 15,
            ...
            if (0 !== g.length) {
                var i = function(a, b) {
                    var c = g.children(a + "#" + b);
                    if (0 !== c.length) {
                        var d = c.attr("id") + "-obj" + ++W;
                        h = h.replace(new RegExp(b,"g"), d)
                    }
                };
                i("path", "raphael-marker-open"),
                i("path", "raphael-marker-block"),
                i("marker", "raphael-marker-endblock33"),
                i("marker", "raphael-marker-endblock55"),
                i("marker", "raphael-marker-endopen77")
            }
            return h
        } catch (j) {
            return c.innerHTML = "",
      ...
      , Y = !0;
    B.hooks.chain("onPreviewRefresh", function() {
        function b() {
            ++c === d && (p(null),
            k.onPreviewFinished(),
            a && n.isEditablePage === !0 && J === !1 && t.populateNoteRemarks(),
            o.closeArticleLoadingAlert())
        }
        $("div.custom-block.details div.summary").on("click", function(a) {
            a.preventDefault();
            let node = $(this).parent();
            node.hasClass("open") ? node.removeClass("open") : node.addClass("open")
        }),
        $("div.sequence-diagram:not(:has(>svg))").each(function() {
            var a = X(this.textContent, "seq");
            "" !== a && (this.innerHTML = a)
        }),
        $("div.flow-diagram:not(:has(>svg))").each(function() {
            var a = X(this.textContent, "flow");
            "" !== a && (this.innerHTML = a)
        }),
        $("div.mermaid:not(:has(>svg))").each(function() {
            var a = $(this);
            mermaid.init(void 0, a),
            com.zybuluo.mdeditor.common.attachMermaidStyle(a)
        }),
        $("div.mindmap:not(:has(>svg))").each(function() {
            var a = $(this);
            mermaid.init(void 0, a)
        }),
        s.renderToc(),
    ...
    var Z = null
      , ab = null
      , bb = null;
    B.hooks.set("insertLinkDialog", function(a) {
        return cb = a, cb("", ""),
        !0
    });
    var cb = null;
    B.hooks.set("insertImageDialog", function(a) {
        return cb = a, cb("", ""),
        !0
    });
    var db = function() {
        ...
        $("#wmd-delete-button > span").addClass("icon-delete muted"),
        $("#wmd-high-button > span").addClass("icon-high muted"),
        $("#wmd-center-button > span").addClass("icon-center muted"),
        $("#wmd-checklist-button > span").addClass("icon-checklist muted"),
        ...
        $("#wmd-redo-button").after('<li id="wmd-editor-save-button" class="wmd-button" title="自动保存" style="width: 70px;"><span style="width: 70px; color: #777; font-size: 14px;"></span></li>'),
        $("#wmd-editor-save-button").css("margin-left", "50px"),
        $("#wmd-editor-small-button").hide(),
        $("#wmd-editor-save-button").hide()
    }
      , eb = !1
      , fb = function(a) {
        var b = j.getValue();
        if (a === b)
            return void window.alert("此历史版本和当前文稿没有差别。");
        var c = m(a, b, !1)
          , d = !1;
        x("当前文稿和历史版本的差别", '请查看当前文稿和历史版本的差别，<span class="insert">绿色高亮</span>代表当前文稿独有的内容，<span class="delete">红色高亮</span>代表历史版本独有的内容，其余为共有内容，点击<strong>【恢复到这个版本】</strong>使用历史版本替换当前文稿（去除绿色部分，保留红色部分）， 或者点击<strong>【取消】</strong>继续使用当前文稿。<br><br>' + c.html(), function() {
            d = !0,
            $("#notification-popup-window").modal("hide"),
            eb = !0,
            N(b, a),
      ...
      , gb = !1
      , hb = function(a) {
        var last = localStorage.getItem("lastMarkdown_data");
        if (last) a = last;
        j.initValue(a);
        var b = null
          , c = function(a) {
            ...
            return function() {
                null === b ? (a(),
                b = "") : (n.isEditablePage === !0 && (gb = !0),
                c())
            }
        };
        window.lightMode ? (B.run(c),
        B.undoManager.reinit(a, 0, 0, 0)) : B.run(j.getEditor(), c),
        db()
    };
    return {
        initEditor: hb,
...
$(function() {
    var a = com.zybuluo.mdeditor.unifiedEditor
      ...
      , v = function(a) {
        ...
        $("#customized-style-css").text(a.textCustomizedCss ? a.textCustomizedCss.replace(/(^|\})([\s\S]+?)\{/g, function(a, b, c) {
            c = c.replace(/[\n]+/g, "");
            var d = []
              , e = c.split(",");
            for (var f in e) {
                "" !== e[f] && d.push(e[f].startsWith("#wmd-preview") || e[f].startsWith(".wmd-preview") ? e[f] : "#wmd-preview " + e[f]);
            }
            return b + "\n" + d.join(", ") + " {"
        }) : "{}")
    }
      , w = function() {
        ...
        function y() {
            Y();
            var b = $(window).height() - $(".preview-container").position().top - 1;
        ...
        var drak = com.zybuluo.mdeditor.common.getCurrentThemeClassName() === 'theme-black';
        console.log("h drak", drak);
        h(".wmd-button > span", (drak ? "#F9F9F5" : "#101010"), "#BBBBBB"),
        h(".preview-button > span", (drak ? "#F9F9F5" : "#101010"), "#BBBBBB"),
        $("a#signup, a#login").on("click", function(b) {
            b.preventDefault();
        ...
        $("#mdeditor-logout-submenu").on("click", function() {
            var a = function() {
                n.clear(function() {
            ...
            b.isSavingInProgress() ? p("警告！", "正在保存文稿，离开当前文稿可能会丢失内容，确定立即离开？", function() {
                b.setSavingInProgressFalse(),
                a()
            }) : a()
        }),
        $("#mdeditor-user-settings-submenu, #user-settings-resend, #user-settings-update-email").on("click", function(a) {
            a.preventDefault(),
            s($(this).attr("href"))
        }),
        ...
        var z = function() {
            j.hideToolBar(),
            j.isHidden() ? ($("#wmd-hidden-menu-button span").removeClass("icon-chevron-sign-up").addClass("icon-chevron-sign-down"),
            $("#wmd-hidden-menu-button").attr("title", "显示工具栏")) : ($("#wmd-hidden-menu-button span").removeClass("icon-chevron-sign-down").addClass("icon-chevron-sign-up"),
            $("#wmd-hidden-menu-button").attr("title", "隐藏工具栏")),
            $(window).resize()
        };
        $("#wmd-hidden-menu-button").on("click", z),
        k.loadDefault(),
        $("#editor-reader-exchange-button").on("click", function() {
            k.exchange()
        });
        var B = function(a) {
            $.get(f.noteRevisionUrl + a, {}, function(a) {
        ...
        $("#preview-info-button").on("show", function() {
            ...
            E.text("历史版本：正在加载中..."),
            $.get(f.noteRevisionsUrl, {}, function(a) {
                if (a.userNoteRevisionUIs) {
                    ...
                    for (var e in c) {
                        ...
                        var i = h.children(".revision-reason").text(C[f.reason])
                          , j = h.children(".revision-time-diff").attr("created_time_diff", f.created_time_diff + "前").text(f.created_time_diff + "前");
                        ("publish" === f.reason || "republish" === f.reason) && (i.addClass("yellow-on-black"),
                        j.addClass("yellow-on-black")),
                        D.append(g),
                        e >= 2 && h.addClass("paid-user-color")
                    }
                    D.children(".revision-item").on("mouseenter", function() {
                        var a = $(this)
                    ...
                    D.children(".revision-item").children("a").on("click", function(a) {
                        a.preventDefault();
                        var c = $(this);
        ...
        $("#preview-file-button").on("show", function() {
            n.get(function(a) {
                if (null !== a) {
                    F.children(".latest-note").remove(),
                    G.text("最近使用：加载中...");
                    var b = [];
                    q.exportAllLocalNotes(a, function(a) {
                        b.push({
                        ...
                        F.children(".latest-note").children("a").on("click", function(a) {
                            a.preventDefault();
        ...
        $("ul#file-list").on("click", "li a", function(a) {
            a.preventDefault();
        ...
        $("#new-offline-file-submenu").on("click", function() {
            e.newFile(!0)
        }),
        $("#open-file-submenu").on("click", function() {
            e.openLocalFile()
        }),
        $("#preview-delete-submenu").on("click", function() {
            e.deleteFile()
        }),
        $("#preview-revision-submenu").on("click", function() {
            e.saveRevision()
        });
        var H = document.getElementById("wmd-panel-editor");
        H.addEventListener("dragover", function(a) {
            a.stopPropagation(),
            a.preventDefault(),
            a.dataTransfer.dropEffect = "copy"
        }, !1),
        H.addEventListener("drop", function(a) {
            a.stopPropagation(),
            a.preventDefault();
            ...
            if (c) {
                ...
                d.onload = function(a) {
                    return function(b) {
                        var c = b.target.result;
                        if (!a.type.match("text.*") && ("" !== a.type || c.match(/\uFFFD/)))
                            return void $("#wmd-image-button").data("droppedFile", a).click();
        ...
        window.desktopGui ? ($("#about-menu").append('<li><a tabindex="-1" href="https://www.zybuluo.com/cmd" target="_blank">客户端版本 v' + window.desktopGui.App.manifest.version + "</a></li>"),
        ...
        $("#reload-current-usernote-submenu").on("click", function() {
            n.get(function(a) {
        ...
        $("#download-all-submenu").on("click", function() {
            e.downloadFile("markdowns", null)
        }),
        $("#download-template-html-submenu").on("click", function() {
            console.log("download-template-html-submenu")
            e.downloadFile("template-html", null)
        }),
        $("#download-print-html-submenu").on("click", function() {
            e.downloadFile("print", null)
        }),
        $("#download-pdf-submenu").on("click", function() {
            e.downloadFile("pdf", null)
        }),
        $("#upload-yinxiang-submenu").on("click", function() {
            e.uploadFile("yinxiang")
        }),
        $("#upload-evernote-submenu").on("click", function() {
        ...
        $("#remove-password-submenu").on("click", function(a) {
            a.preventDefault(),
            e.removePassword()
        }),
        ...
        $("#customized-style-submenu").on("click", function() {
            p("自定义页面样式", "", function() {
                var a = $("#notification-popup-window")
                  , b = a.find("#inputFontSize").val();
                ("" === b || isNaN(b) === !0) && (b = "16");
                ...
                var a = $("#notification-popup-window .modal-body")
                  , b = '<div class="control-group"> <label class="control-label" style="width: 90px" for="${id}"><strong>${label}</strong></label> <div class="controls" style="margin-left: 100px"> <input type="text" id="${id}" ${anyAttribute}> <span class="help-block" style="font-size: 12px; margin-top: 5px;">${help}</span> </div> </div>'
                  , c = b.replace(/\$\{id\}/g, "inputFontSize").replace("${label}", "字号大小").replace("${anyAttribute}", 'value="16"').replace("${help}", "")
                  , d = b.replace(/\$\{id\}/g, "inputEditorFont").replace("${label}", "编辑区字体").replace("${anyAttribute}", 'placeholder="默认字体"').replace("${help}", '示例：Consolas, 幼圆, 黑体 <br> 编辑区请使用<a href="https://zh.wikipedia.org/zh/%E7%AD%89%E5%AE%BD%E5%AD%97%E4%BD%93" target="_blank">等宽字体</a>，否则会引起光标错位。')
                  ...
                  , f = '<div class="control-group"> <label class="control-label" style="width: 90px" for="textCustomizedCss"><strong>自定义 CSS</strong></label> <div class="controls" style="margin-left: 100px"> <textarea id="textCustomizedCss" spellcheck="false" style="width:405px; height:320px; font-family: Monaco, Menlo, Consolas, \'Courier New\', monospace;"></textarea> <span class="help-block" style="font-size: 12px; margin-top: 5px;">CSS 可用于改变预览区的默认样式，查看<a href="https://zh.wikipedia.org/wiki/%E5%B1%82%E5%8F%A0%E6%A0%B7%E5%BC%8F%E8%A1%A8" target="_blank">语法简介</a></span> </div> </div>';
        ...
        $("#menu-doc-add").on("click", function() {
            e.newFile()
        }),
        !window.lightMode) {
            ...
            $("#show-line-number-submenu").on("click", function() {
        ...
        var V = com.zybuluo.common.browserType
          , W = "keydown";
        (V.isOpera || V.isFirefox) && (W = "keypress"),
        $(document).on(W, function(b) {
            if ((b.ctrlKey || b.metaKey) && !b.shiftKey) {
                ...
                case "i":
                    if (b.altKey)
                        if (d.isEditorReader)
                            $("#preview-info-button > span").dropdown("toggle");
                ...
                case "n":
                    if (b.altKey && d.isEditorReader) {
                        $("#preview-doc-button > span").dropdown("toggle");
                        break
                    }
                    return;
                case "u":
        ...
        var X = com.zybuluo.mdeditor.mediumEditor
          , Y = function() {
            c().isEditorReader && (X.deselectText(),
            X.hideToolbarActions())
        };
        ...
        $("#wmd-editor-small-button").on("click", function() {
            g.switchNormalModeFromFullEditorMode(x)
        }),
        $("#preview-reader-expand2-button").hide(),
        $("#preview-reader-expand2-button").on("click", g.switchExpandReaderMode),
        $("#preview-reader-expand-button").on("click", g.switchExpandReaderMode),
        $("#preview-reader-full-button").on("click", g.switchFullReaderMode),
        $("#preview-reader-small-button").on("click", function() {
            g.switchNormalModeFromFullReaderMode(x)
        }),
        $("#preview-reader-small-button").hide(),
        localStorage.getItem("review_isExpand") === "true" ? g.switchExpandReaderMode() : null,
        $(window).resize(Z),
        y()
    }
      , x = function() {
        ...
        b = -1 != d ? d : c.length;
        var e = a.getEditor();
        window.lightMode ? e.caret(b) : (-1 != d ? e.navigateLineEnd() : e.navigateFileEnd(),
        e.focus())
    }
      , y = function() {
        window.desktopGui && $.get(f.cmdDesktopVersionUrl, {}, function(a) {
            a.version && parseFloat(window.desktopGui.App.manifest.version) < parseFloat(a.version) && (com.zybuluo.mdeditor.common.setToastrOptions("toast-bottom-right", !0, "0", "0", function() {
      ...
      , z = null
      , A = function(a, b, c) {
        q.syncServerNoteActions(a, b, function() {
            ...
            return c > 0 ? void A(a, b, c) : (toastr.clear(z),
            z = null,
            q.populateOfflineData(a, null, function(a, c) {
                r(b, c)
            }),
            void setTimeout(function() {
                d.length > 0 ? (com.zybuluo.mdeditor.common.setToastrOptions("toast-bottom-right", !0, "10000", "1000", function() {
                    com.zybuluo.mdeditor.common.reloadCurrentId(d[0])
                }),
                toastr.success("同步已完成，其中 " + d.length + " 篇文稿需要合并版本，点击此处访问文稿并合并。")) : (com.zybuluo.mdeditor.common.setToastrOptions("toast-bottom-right", !1, "5000", "1000", null),
                toastr.success("本地文稿已同步至最新状态。"))
            }, 1500))
        }, function(a, b) {
            toastr.clear(z),
            z = null,
      ...
      , B = function(a, d, e) {
        ...
        b.initEditor(e);
        var h = c();
        if (h.isFullEditor ? (g.switchFullEditorMode(),
        x()) : h.isFullReader ? g.switchFullReaderMode() : h.isHashMode || x(),
        f.isUrlMode === !0) {
